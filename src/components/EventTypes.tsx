...
export function EventTypes() {
  ...
  const getStatsForPeriod = () => ({
    thisMonth: { value: 18, trend: "+12%" },
    thisWeek: { value: 7, trend: "+9%" },
    today: { value: 3, trend: "-2%" }
  });
  ...
  return (
    <div className="min-h-screen bg-background text-foreground">
        ...
        <div className="px-6 py-4">
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center gap-3">
              ...
              <Button
                size="sm"
                className="h-8 px-4 font-light bg-primary hover:bg-primary/90"
              >
      ...
      <div className="px-6 py-6">
        {/* User Profile Card */}
        <Card className="mb-6 border-border/20 bg-card/50 backdrop-blur supports-[backdrop-filter]:bg-card/30">
          <CardContent className="p-6">
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-4">
                <div className="w-12 h-12 bg-gradient-to-br from-primary/80 to-primary rounded-full flex items-center justify-center text-primary-foreground text-lg font-medium">
                  {user.avatar}
                </div>
                <div>
                  <h2 className="text-xl font-light">{user.name}</h2>
                  <p className="text-sm text-muted-foreground">cal.id/{user.username}</p>
                </div>
              </div>
              <div className="flex items-center gap-6">
                <div className="text-center">
                  <div className="flex items-baseline gap-1">
                    <span className="text-2xl font-extralight">{stats.thisMonth.value}</span>
                    <span className="text-xs text-green-500">{stats.thisMonth.trend}</span>
                  </div>
                  <p className="text-xs text-muted-foreground">This Month</p>
                </div>
                <div className="text-center">
                  <div className="flex items-baseline gap-1">
                    <span className="text-2xl font-extralight">{stats.thisWeek.value}</span>
                    <span className="text-xs text-green-500">{stats.thisWeek.trend}</span>
                  </div>
                  <p className="text-xs text-muted-foreground">This Week</p>
                </div>
                <div className="text-center">
                  <div className="flex items-baseline gap-1">
                    <span className="text-2xl font-extralight">{stats.today.value}</span>
                    <span className="text-xs text-red-500">{stats.today.trend}</span>
                  </div>
        ...
        {/* Teams Grid */}
        <div className="mb-6">
          <h3 className="text-lg font-light mb-4">Your Teams</h3>
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            {teams.map((team) => (
              <Card key={team.id} className="border-border/20 bg-card/50 backdrop-blur supports-[backdrop-filter]:bg-card/30 hover:bg-card/60 transition-all duration-200 cursor-pointer">
                <CardContent className="p-4">
                  <div className="flex items-center gap-3">
                    <div className={`w-8 h-8 ${team.color} rounded-lg flex items-center justify-center text-white text-sm font-medium`}>
                      {team.name.charAt(0)}
                    </div>
                    <div className="flex-1 min-w-0">
                      <h4 className="font-medium text-sm truncate">{team.name}</h4>
                      <p className="text-xs text-muted-foreground truncate">{team.url}</p>
                    </div>
